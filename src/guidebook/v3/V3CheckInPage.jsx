...
export default function V3CheckInPage() {
  ...
  // Uses a Cloud Function so it works in incognito (no Firebase Auth needed).
  useEffect(() => {
    const resumeId = new URLSearchParams(window.location.search).get('resume')
    if (!resumeId) return
    const fn = httpsCallable(functions, 'getCheckinResume')
    fn({ docId: resumeId }).then(res => {
      const d = res.data
      if (!d || d.propertySlug !== slug) return
    ...
    }).catch(err => console.error('[Functions] resume load failed:', err))
  }, [slug])
