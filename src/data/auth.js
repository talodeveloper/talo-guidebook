...
// When Firebase reports a signed-out state (explicit sign-out, expired or
// revoked token), clear the guard flags so the UI can't show a logged-in
// shell that can't actually do anything.
onAuthStateChanged(auth, (user) => {
  if (!user) FLAG_KEYS.forEach(k => localStorage.removeItem(k))
})
